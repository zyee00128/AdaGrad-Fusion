...
class AIMT_NN(nn.Module):  ## backbone model definition
    def __init__(self, nOUT, complexity, inputchannel, num_layers=35, rank_list=32, information='fisher',
        ...
        #     Linear(complexity, complexity // 2, r=self.rank_list[-3], information=information, dropout_coef=dropout_coef),
        #     nn.LeakyReLU(),
        #     Linear(complexity // 2, 1, r=0, information=information, dropout_coef=dropout_coef)
        # )
...
class NN_default(nn.Module): ## backbone model definition
    def __init__(self,nOUT,complexity,inputchannel,num_layers=35,rank_list=32,information='fisher',num_encoder_layers=3,dropout_coef=0.2,
                 positive_probably=0.1):
        super(NN_default,self).__init__()

        self.positive_probably = torch.tensor(positive_probably)
        # print('positive probably: ',self.positive_probably)
        ...
        # 辅助分类头：二分类（异常/正常）
        self.auxiliary_classifier = nn.Sequential(
            Linear(complexity, complexity//2, r=self.rank_list[-3],  # 复用rank_list的最后几个值
                  information=information, dropout_coef=dropout_coef),
            nn.LeakyReLU(),
            Linear(complexity//2, 1, r=0,  # 输出1维（二分类）
                  information=information, dropout_coef=dropout_coef)
        )
        # 主分类头
        self.classifier = nn.ModuleList(
    ...
    def forward(self, x, semi_flag = False):
        x = self.feature_extraction(x, semi_flag)
        aux = self.auxiliary_classifier(x)
        # att_aux = torch.sigmoid(aux)
        aux_pred = torch.sigmoid(aux) > self.positive_probably.to(aux.device)  # 动态二分类阈值
        # # print('positive_probably: ', self.positive_probably)
        # # print('aux_pred: ', aux_pred)
        # # print(torch.sigmoid(aux))
        aux_mask = aux_pred.float()
        # # print('aux_mask: ', aux_mask)

        x_main = x
        for layer in self.classifier:
            x_main = layer(x_main) 
        x_main = aux_mask * x_main + (1 - aux_mask) * x_main.detach()  # 使用辅助分类头的输出作为主分类头的加权
        return aux,x_main
